...
- This is the second slice of a session-backed arc (`open`, `peek` now; `cut`, `commit`, `rounds`, `session gc`/lock/doctor land in later slices of the same 0.15.x line). `peek` is the first verb to read spoken content out of a session; `open` stays counts-only.
