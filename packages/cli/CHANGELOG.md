...
- This is the third slice of a session-backed arc (`open`, `peek` before it; `rounds` diff, `session gc`, lock, and `doctor` reporting land in B-V4, the same 0.15.x line). `cut` and `commit` are the first verbs to write to a session rather than only read it.
- No lockfile guards concurrent writers on `proposals.json` or `rounds/` in this slice. `cut` and `commit` both read-modify-write the way every other session file already does, correct for one writer; B-V4's advisory lock closes the gap for two.
- The session store persists `meta.json`, the cached `detect.json`, a copy of the transcript, `refs.json`, and now `proposals.json` and `rounds/round-N/` (an EDL copy and its build report per round). It never persists renders or wavs: the whole session directory is disposable cache, and the EDL a human approves lives where they wrote it, not inside it.
